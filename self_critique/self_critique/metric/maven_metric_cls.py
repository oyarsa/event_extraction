...
def parse_instance(answer: str) -> str:
    """Parse string answer to separate into class and spans.

    Since this is replication of the original FCR tagged format, we hardcode the
    relation to be `cause`, and the `[Effect]` tag is always empty.

    `[Cause] This is a cause [Relation] cause [Effect]`
    """
    if m := re.findall(r"\[Cause\](.*?)\[Relation\].*?\[Effect\].*?$", answer):
        return m[0].strip()
    else:
        return ""
