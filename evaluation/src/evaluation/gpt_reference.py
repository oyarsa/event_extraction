...
class Message:
    ...
    def gpt_msg(self) -> str:
        return "\n\n".join([self.context, self.gold, self.output]).strip()
...
def make_messages(
    sampled_data: list[dict[str, Any]], result_mode: ResultMode
) -> list[Message]:
    ...
    for item in sampled_data:
        ...
        output_entities, _ = parse_instance(item["output"])
        output = (
            "Candidate answer:\n"
            f"Cause: {' | '.join(output_entities['Cause'])}\n"
            f"Effect: {' | '.join(output_entities['Effect'])}\n"
        )
...
def main(
    ...
    messages = make_messages(sampled_data, result_mode)
