    ...
    for batch in tqdm(val_loader, desc=desc):
        ...
        with torch.no_grad():
            ...
            pred_ids = model.generate(
                input_ids, attention_mask=attention_mask, max_new_tokens=1
            )
            pred_texts = tokenizer.batch_decode(pred_ids, skip_special_tokens=True)
            preds.extend(1 if text == "valid" else 0 for text in pred_texts)

            # TODO: A can store this in the dataset instead of decoding every time
            gold_texts = tokenizer.batch_decode(labels, skip_special_tokens=True)
            golds.extend(1 if text == "valid" else 0 for text in gold_texts)
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        results = evaluate(
            ...
            val_loader,
            device,
            desc=f"Evaluation ({epoch + 1}/{num_epochs})",
        )
...
class ClassifierDataset(Dataset):
    ...
    labels: Mapping[str, torch.Tensor] | None
    data: list[DataEntry]
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self.labels is not None:
            d["labels"] = self.labels["input_ids"][idx]
        return d
...
def preprocess_data(
    ...
    if has_labels:
        labels = tokenizer(
            text_target=["valid" if d.valid else "invalid" for d in data],
            padding="max_length",
            truncation=True,
            return_tensors="pt",
            max_length=max_seq_length,
...
def run_training(
    ...
    results = evaluate(
        trained_model, tokenizer, eval_loader, device, desc="Final evaluation"
    )
    metrics = calc_metrics(results)
...
def run_evaluation(
    ...
    results = evaluate(
        model, tokenizer, loader, device, desc=f"{desc.capitalize()} evaluation"
    )
    metrics = calc_metrics(results)
...
def run_inference(
    ...
    results = infer(model, tokenizer, loader, device, desc="Inference")
