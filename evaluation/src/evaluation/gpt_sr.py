...
def run_progressive_refinement(
    ...
    refinements: str,
    num_turns: int,
    result_selection_strategy: ResultSelection,
) -> GptSolveResult:
    total_cost = 0
    ...
    last_solution = solution_log[-1]
    match result_selection_strategy:
        case ResultSelection.LAST:
            result = last_solution.result
        case ResultSelection.MAJORITY:
            result = most_common([s.result for s in solution_log])

    return dataclasses.replace(last_solution, result=result, cost=total_cost)
