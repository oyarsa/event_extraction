...
# max_samples = 100
max_samples = None
output_path = "output/hparam_weak"
do_train = True
do_evaluation = True
do_test = False
do_inferece = False
...
print(f"infer_data_path: {infer_data_path}")
print()

all_metrics: list[dict[str, Any]] = []
total_combinations = len(model_names) * len(learning_rates) * len(max_seq_lengths)

for i, (model_name, learning_rate, max_seq_length) in enumerate(
    tqdm(product(model_names, learning_rates, max_seq_lengths))
):
    batch_size = 16 if max_seq_length == 512 else 32
    print(f"\n\n>>>> {i+1} / {total_combinations} <<<<")
    print(
        f"Model: {model_name}, learning rate: {learning_rate}, max_seq_length:"
        f" {max_seq_length}, batch_size: {batch_size}\n"
    )

    model_name_for_path = model_name.replace("/", ".")
    output_name = f"{model_name_for_path}_{learning_rate}_{max_seq_length}"
